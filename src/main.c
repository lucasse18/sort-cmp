...
int main(int argc, char *argv[]) {
  ...
  int c;
  int index = 0;
  while((c = getopt_long(argc, argv, "ha:k:", opts, &index)) != -1) {
    switch(c) {
    case 0:
      break;
  ...
  for(size_t i = 0; i < 4; i++) {
    ...
    clock_t start = clock();
    check(start != -1, "function 'clock' returned error.");
    ...
    clock_t stop = clock();
    check(stop != -1, "function 'clock' returned error.");

    assert_asc_order(A, n, compare_fn);

    fprintf(stdout, "%zd,%.3f\n", k, 1000 * ((double)(stop - start))/CLOCKS_PER_SEC);
  }
